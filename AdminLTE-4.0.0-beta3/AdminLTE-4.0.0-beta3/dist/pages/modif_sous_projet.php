...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    $image = $_POST['image'];
    $stmt = $pdo->prepare("INSERT INTO sous_projet (id_projet, titre, description, image) VALUES (?, ?, ?, ?)");
    $stmt->execute([$id_projet, $titre, $description, $image]);
    header("Location: modif_sous_projet.php");
    exit;
}

// Supprimer un sous-projet
if (isset($_GET['delete'])) {
    $stmt = $pdo->prepare("DELETE FROM sous_projet WHERE id = ?");
...
$projets = $pdo->query("SELECT * FROM projet")->fetchAll(PDO::FETCH_ASSOC);

// Récupérer tous les sous-projets
$stmt = $pdo->query("SELECT sp.*, p.titre AS projet_titre FROM sous_projet sp JOIN projet p ON sp.id_projet = p.id");
$sous_projets = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<head>
  <meta charset="utf-8" />
  <title>Admin - Modifier Sous-Projets</title>
...
<div class="app-wrapper d-flex">
    ...
    <div class="sidebar-wrapper">
      <nav class="mt-2">
        <ul class="nav sidebar-menu flex-column" role="menu">
          ...
          <li class="nav-item"><a href="./modif_sous_projet.php" class="nav-link active"><i class="nav-icon bi bi-circle"></i><p>modif projet</p></a></li>
  ...
  <main class="app-main flex-grow-1 p-4">
    <div class="container-fluid">
      <h1 class="mb-4">Gérer tous les sous-projets</h1>

      <!-- Formulaire d'ajout -->
      <div class="card mb-4">
        <div class="card-header bg-primary text-white">Ajouter un nouveau sous-projet</div>
        <div class="card-body">
          <form method="POST">
            <div class="mb-3">
              <label class="form-label">Projet concerné</label>
              <select name="id_projet" class="form-control" required>
                <?php foreach ($projets as $projet): ?>
                  <option value="<?= $projet['id'] ?>">#<?= $projet['id'] ?> - <?= htmlspecialchars($projet['titre']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="mb-3">
      ...
      <div class="card">
        <div class="card-header bg-dark text-white">Liste des sous-projets existants</div>
        <div class="card-body table-responsive">
          <table class="table table-bordered table-hover bg-white text-dark">
            ...
            <tbody>
              ...
              <tr>
                <td><?= $sous['id'] ?></td>
                <td>#<?= $sous['id_projet'] ?> - <?= htmlspecialchars($sous['projet_titre']) ?></td>
                <td><?= htmlspecialchars($sous['titre']) ?></td>
                <td><?= htmlspecialchars($sous['description']) ?></td>
                <td><?= htmlspecialchars($sous['image']) ?></td>
                <td>
                  <a href="?delete=<?= $sous['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Supprimer ce sous-projet ?')">🗑 Supprimer</a>
                </td>
              </tr>
              <?php endforeach; ?>
              <?php if (empty($sous_projets)): ?>
              <tr><td colspan="6" class="text-center">Aucun sous-projet trouvé.</td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
